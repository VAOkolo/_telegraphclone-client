...
const url = "https://telegraphclone-server.herokuapp.com/posts"
...
async function submitPost(e){
    ...
    if(title == "" || alias == "" || description == ""){
        ...
        const response = await fetch('https://telegraphclone-server.herokuapp.com/posts', options)
...
async function getNewPostId(postData){
    ...
    let response = await fetch('https://telegraphclone-server.herokuapp.com/posts')
    let data = await response.json();
    let posts = data
    
    console.log(posts)
    let postID = posts[posts.length-1]._id
    console.log(postID)
...
async function getPost(id){
    try {
        const response = await fetch(`https://telegraphclone-server.herokuapp.com/posts/${id}`);
        const data = await response.json();
        return data;
    } catch (err) {
        console.warn(err);
...
async function publishPost(e){
    ...
    fetch(`https://telegraphclone-server.herokuapp.com/posts/${id}`, options)
            .then(r => r.json())
